...
func TestCallbackSerializer_Schedule_FIFO(t *testing.T) {
	...
	// Ensure that the order of scheduling and the order of execution are the
	// same.
	for i := 0; i < numCallbacks; i++ {
		if scheduleOrder[i] != executionOrder[i] {
			t.Fatalf("Mismatch in order of scheduling and execution: %v vs %v", scheduleOrder, executionOrder)
		}
	}
}
