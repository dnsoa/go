...
func NewXID() XID {
	return NewXIDWithTime(UnixTime())
}
...
func (x *XID) UnmarshalText(text []byte) (err error) {
	*x, err = ParseXID(unsafe.String(unsafe.SliceData(text), len(text)))
	return
}
...
func (x *XID) UnmarshalJSON(b []byte) (err error) {
	...
	*x, err = ParseXID(unsafe.String(unsafe.SliceData(b[1:len(b)-1]), len(b)-2))
	return
}
